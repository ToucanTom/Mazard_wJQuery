...
function flipTile2(row, col){
    ...
    if (randNum <= 25) {
        ...
        $("#message").show().html('You have encountered a foe!');
        $("#newFoe").css('background-image', 'url('+foeOptions[index].image +')').show();
        setTimeout(function() {battle(foeOptions[index])}, 2500);
        return;
    }
    tileCountDown--;
    ...
    $('#'+row + "-" + col).css('background-image' ,'url(' + currentTile.image + ')');
    genRotateDivs();
...
function stageTiles() {
    ...
    for (let i = 0; i < setClickableTiles.length; i++) {
        if (setClickableTiles[i].available) {
            $('#'+setClickableTiles[i].location).css('background-image', "url(Media/gameDeck.png)");
...
function rotateCW() {
    ...
    $('#'+currentTile.location).css('background-image','url(' + currentTile.image + ')');
}
function setRotation() {
    ...
    if (items.dropRate >= rand) {
        ...
        let targetElement = currentGameBoard[currentTile.location[0]][currentTile.location[2]];
        if (rand <= 10 && !currentPlayer.hasSword) {
            $('#'+currentTile.location[0] + "-" + currentTile.location[2]).append("<img src = "+items.sword.image+">");
            targetElement.droppedItem = "sword";
        }
        else if (rand <= 50) {
            if(rand%2 === 0){
                $('#'+currentTile.location[0] + "-" + currentTile.location[2]).append("<img src = "+items.key.image+">");
                targetElement.droppedItem = "key";
            }
            else{
                $('#'+currentTile.location[0] + "-" + currentTile.location[2]).append("<img src = "+items.chest.image+">");
                targetElement.droppedItem = "chest";
            }
        }
        else {
            $('#'+currentTile.location[0] + "-" + currentTile.location[2]).append("<img src = "+items.bread.image+">");
